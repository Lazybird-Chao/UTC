 ...
 */
class RloadWorker: public UserTaskBase{
private:
...
public:
	...
	void runImpl(){
		if(__globalThreadId==0){
			std::cout<<"Start rload latency test:(us)"<<std::endl;
		}
		Timer timer;
		double latency = 0.0;
		inter_Barrier();
		for(int size = 1; size <= MAX_MSG_SIZE; size = (size ? size * 2 : 1)){
			...
			if(__processIdInGroup == 0){
				...
				//std::cout<<"local:"<<localdataBuff[size-1]<<" ";
				for(int i=0; i<loop + skip; i++){
					if(i==skip)
						timer.start();
					gdataBuff.rloadblock(1, localdataBuff, alligned_start_index, size);
					//gdataBuff.quiet();
				}
...
int main(int argc, char* argv[]){
	...
	Task<RloadWorker> latencyTest(plist);
