...
void appft(int niter, double *total_time, logical *verified)
{
  ...
  timer_start(2);      
  compute_initial_conditions(NX, NY, NZ, xnt[0][0]);

  CompExp(NX, exp1);
  CompExp(NY, exp2);
  CompExp(NZ, exp3);          
  fftXYZ(1, NX, NY, NZ, xnt[0][0], (dcomplex *)y[0][0], exp1, exp2, exp3);
  timer_stop(2);
  ...
  for (kt = 1; kt <= niter; kt++) {
    ...
    evolve(NX, NY, NZ, xnt[0][0], y[0][0], twiddle[0][0]);
    if (timers_enabled) timer_stop(11);
    if (timers_enabled) timer_start(15);
    fftXYZ(-1, NX, NY, NZ, xnt[0][0], (dcomplex *)xnt[0][0], exp1, exp2, exp3);
    if (timers_enabled) timer_stop(15);
    if (timers_enabled) timer_start(10);
    CalculateChecksum(&sums[kt], kt, NX, NY, NZ, xnt[0][0]);
