...
#ifdef USE_DEBUG_LOG
	PRINT_TIME_NOW(*m_procOstream)
	(*m_procOstream)<<m_numLocalThreads<<" gputask threads for ["<<m_Name<<"] launched on proc "<<m_processRank
			<<std::endl;
#endif
...
void TaskGPU::threadImpl(ThreadRank_t trank,
						ThreadRank_t lrank,
						std::ofstream *output,
						int hardcoreId,
						int gpuToBind
...
//#define SET_CPU_BIND_TO_ALL
	...
	myUtcGpuContext->ctxDestroy();
	delete myUtcGpuContext;
	m_allLocalThreadUtcGpuContexPtr[lrank] = nullptr;
...
int TaskGPU::initImpl(std::function<void()> initHandle){
	std::unique_lock<std::mutex> LCK1(m_jobExecMutex);
...
int TaskGPU::runImpl(std::function<void()> runHandle){
	std::unique_lock<std::mutex> LCK1(m_jobExecMutex);
...
int TaskGPU::execImpl(std::function<void()> execHandle){
	std::unique_lock<std::mutex> LCK1(m_jobExecMutex);
...
/*****************************************/
int TaskGPU::findProperGPU(int localThreadRank){
	return CudaDeviceManager::getCudaDeviceManager().getAvailableGpu(localThreadRank);
}
