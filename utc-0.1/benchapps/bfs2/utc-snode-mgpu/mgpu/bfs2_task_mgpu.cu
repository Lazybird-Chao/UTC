...
void bfsMGPU::initImpl(Node_t *graph_nodes,
	...
	if(__localThreadId ==0){
		...
		memcpy(this->g_spath, shartestPath, total_graph_nodes*sizeof(int));
		...
		this->nextWaveArray = new uint8_t[total_graph_nodes*__numLocalThreads];
		memset(this->nextWaveArray, 0, total_graph_nodes*__numLocalThreads*sizeof(uint8_t));
		this->stopflag_array = new int[__numLocalThreads];
	}
	intra_Barrier();
	int nodesPerThread = total_graph_ndoes / __numLocalThreads;
...
void bfsMGPU::runImpl(double *runtime,
			int blocksize,
			int batch,
			MemType memtype){
	if(__localThreadId == 0){
