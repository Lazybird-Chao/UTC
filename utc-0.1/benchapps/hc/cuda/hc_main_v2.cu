...
FTYPE get_convergence_sqd(FTYPE *current_ptr, FTYPE *next_ptr, int w, int h){
	FTYPE sum = 0.0;
		for(int i=0; i<h; i++){
			for(int j=0; j<w; j++){
				sum += pow(current_ptr[i*w + j]-next_ptr[i*w+j],2);
			}
		}
		return sum;
}
...
int main(int argc, char**argv){
	...
	while(1){
		...
		t1 = getTime();
		double converge = get_convergence_sqd(U_Curr, U_Next, (int)floor(WIDTH/H), (int)floor(HEIGHT/H));
		t2 = getTime();
		hostcompTime += t2-t1;
