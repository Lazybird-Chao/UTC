...
__global__ void render_kernel(
	...
	//if(row==0 && colum%100 ==0)
	//	printf("x: %d, y: %d\n", g_vars_d.xres, g_vars_d.yres);
	//if(row==100 && colum ==100){
		//for(int i = 0;i<g_vars_d.obj_count; i++)
			//printf("%f  %f  %f\n",obj_pos_array[i].x, obj_pos_array[i].y, obj_pos_array[i].z);
			//printf("%f  %f  %f\n",mat[i].refl, mat[i].spow, mat[i].col.x);
	//}
	if(row < g_vars_d.yres && colum < g_vars_d.xres){
		...
		/*if(row==10 && colum%100 ==0)
			printf("%d  %d   %d\n", row, colum, pixels[row*g_vars_d.xres + colum]);
			*/
	}
	__syncthreads();
}
