...
thread_local int crayCPUWorker::local_yres;
thread_local int crayCPUWorker::local_startYresIndex;
...
void crayCPUWorker::runImpl(double runtime[][3], int loop, bool needDoOutput){
	...
	while(iter < loop){
		...
		timer.start();
		if(needDoOutput && m_cdtOut != nullptr){
...
void crayCPUWorker::render_scanline(int xsz, int ysz, int sl, uint32_t *fb, int samples){
	int i, s;
...
vec3_t crayCPUWorker::trace(ray_t ray, int depth) {
	vec3_t col;
...
int crayCPUWorker::ray_sphere(const sphere2_t *sph, ray_t ray, spoint_t *sp) {
...
vec3_t crayCPUWorker::shade( sphere2_t *obj, spoint_t *sp, int depth) {
	int i;
	vec3_t col = {0,0,0};
...
ray_t crayCPUWorker::get_primary_ray(int x, int y, int sample) {
	ray_t ray;
	FTYPE m[3][3];
...
vec3_t crayCPUWorker::get_sample_pos(int x, int y, int sample) {
	vec3_t pt;
	static FTYPE sf = 0.0;
...
inline vec3_t crayCPUWorker::jitter(int x, int y, int s) {
	vec3_t pt;
 ...
 * some help routine
 */
inline vec3_t crayCPUWorker::reflect(vec3_t &v, vec3_t &n) {
	vec3_t res;
...
inline vec3_t crayCPUWorker::cross_product(vec3_t &v1, vec3_t &v2) {
	vec3_t res;
