...
int main(int argc, char** argv){
	...
	extern char* optarg;
	extern int optind;
	opt=getopt(argc, argv, "vt:p:s:o:b:");
	while(opt!=EOF){
		switch(opt){
		case 'v':
			printTime = true;
	...
	for(int i = 0; i < procs; i++)
			plist.push_back(i);
	Task<MatrixMulWorker<FTYPE>> mm(plist, TaskType::gpu_task);
	mm.init(matrixA, matrixB, matrixC, matrixSize, matrixSize, matrixSize, blockSize);
	mm.run(runtime_m);
	mm.wait();
	/*
	int error =0;
